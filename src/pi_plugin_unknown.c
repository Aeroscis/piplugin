...
PI_PLUGIN_API int pi_plugin_api_version_compatible(uint32_t host_version, uint32_t plugin_version)
{
    ...
    if (PI_PLUGIN_API_VERSION_MAJOR(host_version) != PI_PLUGIN_API_VERSION_MAJOR(plugin_version))
    {
        return 0;
    }
...
{
    PiPluginCapability const* cap = pi_plugin_descriptor_find_capability(desc, iid);
                                                             ...
                                                             char const*               key)
{
    ...
    if (PI_PLUGIN_API_VERSION_MINOR(desc->api_version) < 3)
    {
        return NULL; /* pre-0.3 layout */
    }
    if (!desc->properties)
    {
        return NULL;
