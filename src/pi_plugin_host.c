...
PI_PLUGIN_API PiPluginModule* pi_plugin_module_load(char const* path)
{
    if (!path)
    {
        snprintf(g_load_error, sizeof(g_load_error), "null path");
        return NULL;
    }
...
typedef struct PiPluginDefaultHost {
    ...
    void*                   user_data;
    PiNativeWindow volatile ui_window; /* PI_INVALID_WINDOW = headless  */
...
static PiResult PI_CALL       pi_plugin_default_host_qi(void* self_ptr, PiGuid const* iid, void** out);
...
static PiResult PI_CALL pi_plugin_default_host_ui_qi(void* self_ptr, PiGuid const* iid, void** out);
...
static IPiPluginHostServicesVtbl const s_default_host_services_vtbl = {
...
static IPiPluginHostUIVtbl const s_default_host_ui_vtbl = {
...
static PiResult PI_CALL pi_plugin_default_host_qi(void* self_ptr, PiGuid const* iid, void** out)
{
    ...
    if (pi_guid_equal(iid, &PI_PLUGIN_IID_HOST_UI) && me->ui_window != PI_INVALID_WINDOW)
    {
        ...
        pi_refcounted_init_with_destroy(&ui->base,
                                        (IPiUnknownVtbl const*)&s_default_host_ui_vtbl,
    ...
    IPiPluginHostServices** out_services)
{
    ...
    *out_services = NULL;
    ...
    pi_refcounted_init_with_destroy(&host->base,
                                    (IPiUnknownVtbl const*)&s_default_host_services_vtbl,
                                                        ...
                                                        PiNativeWindow         window)
{
    ...
    if (!host || host->base.unk.lpVtbl != (IPiUnknownVtbl const*)&s_default_host_services_vtbl)
    {
...
{
    if (!dll_path || !class_guid || !out_plugin)
    {
        return PI_E_INVALIDARG;
    }
