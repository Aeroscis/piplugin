...
static PiGuid const EXAMPLE_SERVICE_CLASS_GUID =
...
static PiResult ServiceDoStop(ExampleService* me)
{
    ...
    if (me->running)
    {
        printf("[example service] stopping after %u poll(s)\n", (unsigned)me->polls);
    }
    me->running = 0;
...
static PiResult PI_CALL Service_Poll(void* self_ptr)
{
    ...
    if (!me->running)
    {
        return PI_FAIL; /* do not pretend to work when stopped */
    }
...
static PiResult PI_CALL Plugin_Qi(void* self_ptr, PiGuid const* iid, void** out)
{
    ...
    if (pi_guid_equal(iid, &PI_PLUGIN_IID_SERVICE))
    {
        ...
        pi_refcounted_init_with_destroy(&ifc->base, (IPiUnknownVtbl const*)&s_service_vtbl,
                                        &Ifc_Destroy);
        ifc->owner = me;
...
static PiResult PI_CALL Plugin_Term(void* self_ptr)
{
    ...
    if (me->running)
    {
        ServiceDoStop(me); /* stop before the module goes away */
    }
    return PI_OK;
}
                                       ...
                                       IPiPluginBase** out)
{
    ...
    *out = NULL;
    if (!pi_guid_equal(guid, &EXAMPLE_SERVICE_CLASS_GUID))
    {
        return PI_E_NOINTERFACE;
    }
