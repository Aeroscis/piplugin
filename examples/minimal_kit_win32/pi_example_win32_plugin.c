...
static const PiGuid EXAMPLE_WIN32_CLASS_GUID =
...
static void Plugin_Destroy(void* self)
{
    ...
    if (me->host_ui) { pi_iunknown_release((IPiUnknown*)me->host_ui); me->host_ui = NULL; }
...
/* ---- factory + entry point ---------------------------------------------- */
typedef struct Win32Factory { PiRefCountedBase base; } Win32Factory;
                                       ...
                                       IPiPluginBase** out)
{
    ...
    *out = NULL;
    if (!pi_guid_equal(guid, &EXAMPLE_WIN32_CLASS_GUID)) return PI_E_NOINTERFACE;
