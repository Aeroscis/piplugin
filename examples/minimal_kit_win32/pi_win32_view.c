...
static void PiPluginWin32Trace(char const* fmt, ...)
{
                                                 ...
                                                 WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_ERASEBKGND:
        if (me)
        {
            return 1; /* the paint callback owns the pixels */
        }
        break;
    default:
        break;
 ...
 * -------------------------------------------------------------------------- */
static uint32_t PI_CALL Win32View_AddRef(void* self)
{
    return pi_refcounted_add_ref(self);
}
static uint32_t PI_CALL Win32View_Release(void* self)
{
    return pi_refcounted_release(self);
}

static PiResult PI_CALL Win32View_Qi(void* self_ptr, PiGuid const* iid, void** out)
{
    if (!out)
    {
...
static PiResult PI_CALL Win32View_Attach(void* self_ptr, PiNativeWindow parent)
{
    ...
    if (me->attached)
    {
        return PI_FAIL; /* attach once per detach */
    }
    ...
    if (me->desc.retain)
    {
        me->desc.retain(me->desc.user_data);
    }
...
static PiResult PI_CALL Win32View_Detach(void* self_ptr)
{
    ...
    if (me->desc.release)
    {
        me->desc.release(me->desc.user_data);
    }
    return PI_OK;
}
...
static PiResult PI_CALL Win32View_SetVisible(void* self_ptr, int32_t visible)
{
    ...
    if (me->hwnd)
    {
        ShowWindow(me->hwnd, visible ? SW_SHOW : SW_HIDE);
    }
    return PI_OK;
}

static IPiPluginViewVtbl const s_win32_view_vtbl = {
...
PiResult pi_plugin_win32_view_create(PiPluginWin32ViewDesc const* desc, IPiPluginView** out_view)
{
    ...
    *out_view = NULL;
    ...
    pi_refcounted_init_with_destroy(&me->base, (IPiUnknownVtbl const*)&s_win32_view_vtbl,
                                    &Win32View_Destroy);
    me->desc  = *desc;
