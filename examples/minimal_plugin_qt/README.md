...
点击按钮会更新标签并发 `pi_host_post_message(0x8000)`，宿主打印出来。

## 值得抄的几点

- **套件是 SHARED 库**（roadmap APP-08）：插件运行时必须能找到 `piplugin_qt<后缀>.dll`
  （本仓库的构建会把它部署到 `bin/<CONFIG>/`，别人的部署也要带上）；
- `pi_terminate()` 里调 `pi_qt_view_shutdown_owner(this)`：套件是进程共享的，
  不带 owner 的 `pi_qt_view_shutdown()` 会把**别的** Qt 插件的控件一起拆掉；
