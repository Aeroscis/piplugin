...
#define PI_EXAMPLE_CONTAINER_LEFT 16
#define PI_EXAMPLE_RUN_MS         1500   /* how long to drive the plugin */
...
static LRESULT WINAPI WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_SIZE:
        ...
        if (g_container)
            SetWindowPos(g_container, NULL, PI_EXAMPLE_CONTAINER_LEFT, PI_EXAMPLE_CONTAINER_LEFT,
                         LOWORD(lparam) - 2 * PI_EXAMPLE_CONTAINER_LEFT,
                         HIWORD(lparam) - 2 * PI_EXAMPLE_CONTAINER_LEFT,
                         SWP_NOZORDER);
        /* Forward the size to the plugin's view (the view is not ours to resize). */
        if (g_session && g_slot != PI_HOST_SESSION_INVALID_SLOT) {
            IPiPluginView* view = pi_host_session_get_view(g_session, g_slot);
            if (view && LOWORD(lparam) > 0 && HIWORD(lparam) > 0)
                pi_view_on_resize(view, (int32_t)LOWORD(lparam), (int32_t)HIWORD(lparam));
        }
        return 0;
    case WM_CLOSE:
        DestroyWindow(hwnd);
...
int main(int argc, char** argv)
{
    ...
    wc.lpszClassName = L"PiExampleMinimalHost";
    RegisterClassExW(&wc);
     ...
     *   pi_host_session_require(g_session, &MY_APP_PROTOCOL_IID);
     * see examples/specialized_app for that in practice. */

    /* --- 3) load: module + capability gate + instantiate + initialize ------ */
    hr = pi_host_session_load(g_session, dll, &g_slot);
    if (PI_FAILED(hr)) {
        printf("load failed (hr=%d): %s\n", (int)hr, pi_host_session_last_error(g_session));
        exit_code = 1;
    } else {
        ...
        /* --- 4a) a view: attach it to OUR container and pump it ------------- */
        {
            IPiPluginView* view = pi_host_session_get_view(g_session, g_slot);
            if (view) {
                hr = pi_host_session_attach_view(g_session, g_slot,
                ...
                printf("view: %s (native window %p)\n",
                       ...
                       (void*)(uintptr_t)pi_view_get_native_window(view));
        ...
        {
            IPiService* service = pi_host_session_get_service(g_session, g_slot);
            if (service) {
                hr = pi_service_start(service, NULL, 0);
        ...
        start = GetTickCount();
        while (GetTickCount() - start < PI_EXAMPLE_RUN_MS) {
            MSG msg;
             ...
             * the host's decision. */
            if (g_slot != PI_HOST_SESSION_INVALID_SLOT)
                pi_host_session_drive_idle(g_session);
            {
                IPiService* service = pi_host_session_get_service(g_session, g_slot);
                if (service) pi_service_poll(service);
            }
            Sleep(10);
        }
