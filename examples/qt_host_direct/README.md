...
`QApplication` 并从宿主的 `pi_on_idle()` 里驱动 Qt。宿主自己已经有 `QApplication`
时这条路径**不可用**——套件在 `piqt_app_create()` 里就会撞上宿主的实例（Qt 只允许
一个 application object），`PI_QT_VIEW_TRACE=1` 时日志只到 `attach: enter` 就不再前进。
...
`pi_host_session_require()` 声明了"本宿主的插件必须实现我的 widget 协议"，门禁在
...
2. `pi_qt_direct_host.cpp`：① 自己建 `QApplication` 与 `QLayout`；②
   `pi_host_session_require()` 声明生态要求；③ 加载后 QI 到协议；
   ④ `layout->addWidget(pi_qt_direct_create_widget(...))` —— 集成就这一行；
