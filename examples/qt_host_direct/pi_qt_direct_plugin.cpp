...
private:
    ...
    static const IQtDirectWidgetVtbl s_vtbl;
...
class QtDirectPlugin {
public:
    QtDirectPlugin() : m_base(), m_host(nullptr), m_clicks(0), m_live_widgets(0)
    {
    ...
    void DestroyWidget(QWidget* widget)
    {
        ...
        printf("[qt-direct plugin] destroying widget %p (host is done with it)\n",
               (void*)widget);
        if (m_live_widgets > 0) --m_live_widgets;
...
const IPiPluginBaseVtbl QtDirectPlugin::s_vtbl = {
    { &QtDirectPlugin::Qi, &pi_refcounted_add_ref, &pi_refcounted_release },
    &QtDirectPlugin::InitThunk, &QtDirectPlugin::TermThunk, &QtDirectPlugin::GetViewThunk
};

/* ---- the protocol wrapper ---- */

QtDirectWidgetIfc::QtDirectWidgetIfc(QtDirectPlugin* owner) : m_base(), m_owner(owner)
{
    pi_refcounted_init_with_destroy(&m_base, (const IPiUnknownVtbl*)&s_vtbl, &Destroy);
    if (m_owner) pi_refcounted_add_ref(&m_owner->m_base);
}
...
{
    QtDirectWidgetIfc* ifc = (QtDirectWidgetIfc*)self_ptr;
    if (ifc->m_owner) ifc->m_owner->DestroyWidget(widget);
}

void QtDirectWidgetIfc::Destroy(void* self)
{
    QtDirectWidgetIfc* ifc = (QtDirectWidgetIfc*)self;
    if (ifc->m_owner) { pi_refcounted_release(&ifc->m_owner->m_base); ifc->m_owner = nullptr; }
    delete ifc;
}

const IQtDirectWidgetVtbl QtDirectWidgetIfc::s_vtbl = {
    { &QtDirectWidgetIfc::Qi, &pi_refcounted_add_ref, &pi_refcounted_release },
    &QtDirectWidgetIfc::CreateWidgetThunk, &QtDirectWidgetIfc::DestroyWidgetThunk
};
...
const IPiPluginFactoryVtbl QtDirectFactory::s_vtbl = {
    { &QtDirectFactory::Qi, &pi_refcounted_add_ref, &pi_refcounted_release },
    &QtDirectFactory::GetDescriptor, &QtDirectFactory::GetClassCount,
    &QtDirectFactory::GetClassGuid, &QtDirectFactory::CreateInstance
};
