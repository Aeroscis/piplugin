...
static int VersionCompare(char const* a, char const* b)
{
    int i;
    for (i = 0; i < 4; ++i)
    {
        ...
        if (!ha && !hb)
        {
            break; /* nothing numeric left on either side */
        }
    }
    return 0;
}

static void ScanFile(PiPluginHostSession* session, char const* directory, char const* file)
{
    ScanEntry*                e;
    ...
    if (g_entry_count >= SCAN_MAX_ENTRIES)
    {
        return;
    }
    ...
    if (desc)
    {
        ...
        for (i = 0; i < desc->capability_count; ++i)
        {
            ...
            if (cap->flags & PI_PLUGIN_CAP_PROVIDES)
            {
                ++e->provides;
            }
            ...
            if (cap->flags & PI_PLUGIN_CAP_OPTIONAL)
            {
                ++e->optional;
            }
        }
...
static void ScanDirectory(PiPluginHostSession* session, char const* directory)
{
    WIN32_FIND_DATAA fd;
    HANDLE           h;
...
static void PrintVersionSelection(void)
{
    ...
    for (i = 0; i < g_entry_count; ++i)
    {
        ScanEntry const* winner;
        uint32_t         candidates = 0;

        if (!g_entries[i].usable)
        {
            continue;
        }
        ...
        for (j = 0; j < i; ++j)
        {
            if (g_entries[j].usable && strcmp(g_entries[j].name, g_entries[i].name) == 0)
            {
                break;
            }
        }
        ...
        {
            if (!g_entries[j].usable)
            {
                continue;
            }
            if (strcmp(g_entries[j].name, winner->name) != 0)
            {
                continue;
            }
            ++candidates;
            if (VersionCompare(g_entries[j].version, winner->version) > 0)
            {
                winner = &g_entries[j];
            }
        }
    ...
    if (!any)
    {
        printf("no descriptor name appears twice - nothing to choose\n");
    }
...
{
    char const*            directory = (argc > 1 && argv[1]) ? argv[1] : ".";
    ...
    if (PI_FAILED(hr))
    {
        printf("FATAL: host services (hr=%d)\n", (int)hr);
        return 1;
    }
