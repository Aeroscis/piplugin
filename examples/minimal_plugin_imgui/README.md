...
窗口里会出现一行文字、一个滑块和一个按钮；按按钮会 `pi_host_post_message(0x8000)`，
宿主打印 `[plugin message] msg=0x8000 wparam=N`。

## 值得抄的几点

- 能力声明：`PI_IID_PLUGIN_VIEW (PROVIDES)` + `PI_IID_HOST_UI (OPTIONAL)`；
  headless 宿主下 `QueryInterface(PI_IID_HOST_UI)` 失败 → **不创建 view**（本插件在
