...
static char const* PI_CALL AppInfo_Name(void* self_ptr)
{
...
static PiResult PI_CALL AppInfo_Qi(void* self_ptr, PiGuid const* iid, void** out)
{
    if (!out)
    {
...
static uint32_t PI_CALL AppInfo_AddRef(void* self)
{
    return pi_refcounted_add_ref(self);
}
static uint32_t PI_CALL AppInfo_Release(void* self)
{
    return pi_refcounted_release(self);
}

static IMyAppInfoVtbl const s_app_info_vtbl = {
...
static PiResult PI_CALL AppExtraQi(void* ctx, PiGuid const* iid, void** out)
{
    (void)ctx;
...
static void OnSessionLog(void* user_data, char const* message)
{
    (void)user_data;
    printf("[kit] %s\n", message);
}
...
static int LoadAndCheck(char const* dll, IMyAppJobQueue** out_jobs)
{
...
{
    char const* good_dll = (argc > 1) ? argv[1] : "pi_plugin_example_specialized_plugin.dll";
    char const* bad_dll  = (argc > 2) ? argv[2] : "pi_plugin_example_service.dll";
    ...
    pi_refcounted_init(&s_app_info.base, (IPiUnknownVtbl const*)&s_app_info_vtbl);
