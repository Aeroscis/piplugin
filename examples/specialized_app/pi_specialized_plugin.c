...
static PiGuid const SPECIALIZED_CLASS_GUID =
...
static uint32_t PI_CALL Ifc_AddRef(void* self)
{
    return pi_refcounted_add_ref(self);
}
static uint32_t PI_CALL Ifc_Release(void* self)
{
    return pi_refcounted_release(self);
}
 ...
 * -------------------------------------------------------------------------- */
static PiResult PI_CALL Job_Submit(void* self_ptr, char const* job_name, int32_t* out_job_id)
{
    JobQueueIfc* ifc = (JobQueueIfc*)self_ptr;
    JobPlugin*   me  = ifc ? ifc->owner : NULL;

    if (!me || !job_name || !out_job_id)
    {
...
static PiResult PI_CALL Job_Qi(void* self_ptr, PiGuid const* iid, void** out)
{
    if (!out)
    {
        return PI_E_INVALIDARG;
...
static void Ifc_Destroy(void* self)
{
    ...
    if (ifc->owner)
    {
        pi_iunknown_release((IPiUnknown*)&ifc->owner->base);
        ifc->owner = NULL;
    }
    free(ifc);
}

static IMyAppJobQueueVtbl const s_job_vtbl = {
...
static PiResult PI_CALL Plugin_Qi(void* self_ptr, PiGuid const* iid, void** out)
{
    ...
    if (pi_guid_equal(iid, &MY_APP_JOB_IID))
    {
        ...
        pi_refcounted_init_with_destroy(&ifc->base, (IPiUnknownVtbl const*)&s_job_vtbl, &Ifc_Destroy);
        ifc->owner = me;
...
static void Plugin_Destroy(void* self)
{
    ...
    if (me->app_info)
    {
        pi_iunknown_release((IPiUnknown*)me->app_info);
        me->app_info = NULL;
    }
    if (me->host)
    {
                                       ...
                                       IPiPluginBase** out)
{
    ...
    *out = NULL;
    if (!pi_guid_equal(guid, &SPECIALIZED_CLASS_GUID))
    {
        return PI_E_NOINTERFACE;
    }
...
PI_PLUGIN_ENTRY_DECL
{
    ...
    *out_factory = NULL;
    ...
    if (!s_initialized)
    {
        ...
        s_caps[0].iid   = MY_APP_JOB_IID;
        s_caps[0].flags = PI_PLUGIN_CAP_PROVIDES;
        s_caps[1].iid   = MY_APP_INFO_IID;
