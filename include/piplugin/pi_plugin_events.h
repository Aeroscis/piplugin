 ...
 * pi_plugin_host_post_message() (three integers, no topic, nothing to route on), and a
 * host can only wait for the plugin's next pi_plugin_on_idle()/pi_plugin_on_resize(). Neither
 ...
 *
 *   IPiPluginEventSink   - implemented by a PLUGIN (optional). The host queries it
 *                    after instantiation and pushes addressed events into it.
 *   IPiPluginHostEvents  - provided by a HOST (optional). Plugins query it on the host
 ...
 * side carries on (declare the capability PI_PLUGIN_CAP_OPTIONAL to say so). That is
 * the same runtime negotiation IPiPluginHostUI and IPiPluginService already use.
 *
 * ---------------------------------------------------------------------------
 * Contract (frozen once released - see docs/design/events.md for the RFC these
 * decisions come from):
 *
 *   Threading. pi_plugin_host_events_publish() may be called from ANY thread; the host
 *   marshals it to its own main/owner thread - the thread that called
 *   pi_plugin_initialize() - and everything else (pi_plugin_event_deliver, subscription
 ...
 *   pi_plugin_qt_view_shutdown_owner() uses.
 *
 ...
 *   passes NULL.
 *
 *   Future growth. PiPluginEvent's field set is FROZEN: unlike PiPluginDescriptor
 ...
 *   richer event later means a NEW interface (IPiPluginEventSink2 with PiPluginEvent2), not
 *   another field here.
 * ---------------------------------------------------------------------------
 */
...
/* --------------------------------------------------------------------------
 * IPiPluginEventSink - a plugin that wants to be told things
 * -------------------------------------------------------------------------- */
...
typedef struct IPiPluginEventSinkVtbl {
     ...
     * between pi_plugin_initialize() and pi_plugin_terminate(), and only while the plugin
     * object is alive.
     *
...
/* --------------------------------------------------------------------------
 * IPiPluginHostEvents - a host that routes events
 * -------------------------------------------------------------------------- */
...
typedef struct IPiPluginHostEventsVtbl {
     ...
     * APP-08's pi_plugin_qt_view_shutdown_owner() does). NULL means "the host itself" -
