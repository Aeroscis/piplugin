 ...
 * Diagnostics
 * -------------------------------------------------------------------------- */
static void Dx11Log(PiPluginHostDx11Device* dx, char const* fmt, ...)
{
    char    buf[512];
    va_list ap;
    if (!dx || !dx->log)
...
{
    if (!dx || !dx->swap_chain || !dx->context || width == 0 || height == 0)
    {
        return 0;
    }
    if (dx->scaling_none)
    {
        return Dx11EnsureAtLeast(dx, width, height);
    }
     ...
     * DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT。 */
    {
            ...
            SUCCEEDED(adapter->GetParent(IID_PPV_ARGS(&factory))))
        {
            ...
            if (SUCCEEDED(hr) && swap_chain1)
            {
                ...
                dx->present_model = dx->scaling_none ? "FLIP_DISCARD+latency+scale:none" : "FLIP_DISCARD+latency+scale:stretch";
