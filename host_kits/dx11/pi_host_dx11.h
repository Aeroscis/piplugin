 ...
 *   - 容器矩形由宿主给定（pi_host_dx11_create_embed_container 只负责
 ...
 *   7. 宿主自己的顶层窗口必须带 WS_CLIPCHILDREN（用 pi_host_dx11_top_level_style()），
...
typedef struct PiHostDx11Device PiHostDx11Device;

/* 日志回调：本层把交换链创建与 resize 的关键事实经此上报，去向由宿主决定
 * （测试宿主写文件，脚本按日志模式断言）。 */
typedef void (*PiHostDx11LogProc)(void* user_data, const char* message);

typedef struct PiHostDx11Desc {
 ...
 * -------------------------------------------------------------------------- */
ID3D11Device*           pi_host_dx11_device(PiHostDx11Device* dx);
ID3D11DeviceContext*    pi_host_dx11_context(PiHostDx11Device* dx);
IDXGISwapChain*         pi_host_dx11_swap_chain(PiHostDx11Device* dx);
ID3D11RenderTargetView* pi_host_dx11_render_target(PiHostDx11Device* dx);

/* 实际拿到的呈现模型（诊断用）。present_model 形如
 * "FLIP_DISCARD+latency+scale:none" / "...scale:stretch" / "DISCARD(bitblt)"。 */
const char* pi_host_dx11_present_model(PiHostDx11Device* dx);
int         pi_host_dx11_is_flip_model(PiHostDx11Device* dx);
int         pi_host_dx11_has_scaling_none(PiHostDx11Device* dx);
...
unsigned pi_host_dx11_resize_failures(PiHostDx11Device* dx);
