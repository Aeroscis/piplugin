 ...
 * Small helpers
 * -------------------------------------------------------------------------- */
static char* RouterStrdup(char const* s)
{
    size_t n;
    char*  copy;
    if (!s)
...
static int RouterCopyEvent(PiPluginQueuedEvent* dst, PiPluginEvent const* src)
{
    uint32_t i;
...
static uint32_t RouterFindSubscription(PiPluginEventRouter* router, uint32_t handle)
{
    ...
    {
        if (router->subs[i].handle == handle)
        {
            return i;
        }
    }
 ...
 * -------------------------------------------------------------------------- */
static PiResult PI_CALL Router_Publish(void* self_ptr, PiPluginEvent const* event)
{
    PiPluginEventRouter* me = (PiPluginEventRouter*)self_ptr;
    PiPluginQueuedEvent* slot;
    size_t               topic_len;

    if (!me || !event || !event->topic || !event->topic[0])
    {
        return PI_E_INVALIDARG;
    }
    topic_len = strlen(event->topic);
    if (topic_len >= PI_PLUGIN_EVENT_TOPIC_MAX)
    {
        return PI_E_INVALIDARG; /* documented limit */
    }
...
static PiResult PI_CALL Router_Subscribe(void* self_ptr, char const* topic, void* owner,
                                         ...
                                         uint32_t* out_subscription)
{
    ...
    if (!me || !topic || !topic[0] || !callback || !out_subscription)
    {
        return PI_E_INVALIDARG;
    }
    if (strlen(topic) >= PI_PLUGIN_EVENT_TOPIC_MAX)
    {
        return PI_E_INVALIDARG;
    }
    ...
    if (sub->handle == PI_PLUGIN_EVENT_INVALID_SUBSCRIPTION)
    {
        sub->handle = ++me->next_handle;
    }
    ++me->sub_count;
...
static PiResult PI_CALL Router_Unsubscribe(void* self_ptr, uint32_t subscription)
{
    ...
    if (!me || subscription == PI_PLUGIN_EVENT_INVALID_SUBSCRIPTION)
    {
        return PI_E_INVALIDARG;
    }
...
static uint32_t PI_CALL Router_AddRef(void* self_ptr)
{
    return pi_refcounted_add_ref(self_ptr);
}
static uint32_t PI_CALL Router_Release(void* self_ptr)
{
    return pi_refcounted_release(self_ptr);
}

static PiResult PI_CALL Router_Qi(void* self_ptr, PiGuid const* iid, void** out)
{
    if (!out)
    {
        return PI_E_INVALIDARG;
...
static IPiPluginHostEventsVtbl const s_router_vtbl = {
...
static void Router_Destroy(void* self_ptr)
{
    ...
    free(me->queue);
    for (i = 0; i < me->sub_count; ++i)
    {
        free(me->subs[i].topic);
    }
    free(me->subs);
...
PiResult pi_plugin_event_router_create(PiPluginEventRouter** out_router)
{
    ...
    *out_router = NULL;
    ...
    pi_refcounted_init_with_destroy(&router->base, (IPiUnknownVtbl const*)&s_router_vtbl,
                                    &Router_Destroy);
    *out_router = router;
    return PI_OK;
}
...
void pi_plugin_event_router_set_capacity(PiPluginEventRouter* router, uint32_t capacity)
{
    ...
    if (capacity == 0)
    {
        capacity = PI_PLUGIN_EVENT_ROUTER_DEFAULT_CAPACITY;
    }
