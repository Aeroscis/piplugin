...
static const PiGuid SERVICE_CLASS_GUID =
...
static uint32_t PI_CALL Service_AddRef(void* self_ptr) { return pi_refcounted_add_ref(self_ptr); }
static uint32_t PI_CALL Service_Release(void* self_ptr) { return pi_refcounted_release(self_ptr); }
                               ...
                               const PiPluginServiceOption* options, uint32_t option_count)
{
    ...
    if (!slot || !slot[0]) return PI_E_MISSINGCAPABILITY;

    /* 已经在跑：幂等成功，不重置计数。 */
    if (plugin->running) return PI_OK;

    {
        size_t n = strlen(slot);
        if (n >= sizeof(plugin->slot)) n = sizeof(plugin->slot) - 1;
                                      ...
                                      uint32_t option_count)
{
    ...
    if (!me || !me->owner) return PI_E_INVALIDARG;
...
{
    ServiceIfc* me = (ServiceIfc*)self_ptr;
    if (!me || !me->owner || !out_status) return PI_E_INVALIDARG;
...
static PiResult PI_CALL Plugin_Initialize(void* self_ptr, IPiPluginHostServices* host)
{
    ...
    if (me->host) return PI_OK;              /* 幂等（见 docs/tutorial/write-plugin.md） */
...
static PiResult PI_CALL Plugin_Terminate(void* self_ptr)
{
     ...
     * 免得给宿主多报一次 stop。 */
    if (me && me->running) ServiceDoStop(me);
    return PI_OK;
}
                                               ...
                                               IPiPluginHostServices* host, IPiPluginBase** out)
{
    ...
    *out = NULL;
    if (!pi_guid_equal(guid, &SERVICE_CLASS_GUID)) return PI_E_NOINTERFACE;
