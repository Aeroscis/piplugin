 ...
 * pi_guid_equal
 * -------------------------------------------------------------------------- */
static PiGuid const GUID_A   = PI_GUID(0x11223344, 0x5566, 0x7788,
                                       0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00);
static PiGuid const GUID_A2  = PI_GUID(0x11223344, 0x5566, 0x7788,
                                       0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00);
/* 只有 data1 不同 */
static PiGuid const GUID_B   = PI_GUID(0x11223345, 0x5566, 0x7788,
                                       0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00);
/* 前三个字段相同，只有 data4 不同 */
static PiGuid const GUID_C   = PI_GUID(0x11223344, 0x5566, 0x7788,
                                       0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x01);
/* 只有 data2/data3 不同 */
static PiGuid const GUID_D   = PI_GUID(0x11223344, 0x5567, 0x7789,
                                       0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x00);
/* app 侧接口 GUID（随机风格，见 docs/design/interfaces.md 5.1） */
static PiGuid const GUID_APP = PI_GUID(0x9F3C1D42, 0x7B08, 0x4E55,
...
static void TestDescriptorHelpers(void)
{
    ...
    caps[0].iid           = PI_PLUGIN_IID_PLUGIN_VIEW;
    caps[0].flags         = PI_PLUGIN_CAP_PROVIDES;
    caps[1].iid           = PI_PLUGIN_IID_HOST_UI;
    caps[1].flags         = PI_PLUGIN_CAP_OPTIONAL;
    caps[2].iid           = GUID_APP;
    caps[2].flags         = PI_PLUGIN_CAP_REQUIRED;
...
static void TestDescriptorProperties(void)
{
    ...
    props[0].key   = "com.example.kind";
    props[0].value = "qt-plugin";
    props[1].key   = "com.example.blank";
    props[1].value = ""; /* 空值合法 */
    props[2].key   = "UTF8.\xE9\x94\xAE";
    props[2].value = "\xE5\x80\xBC"; /* UTF-8 键与值 */
...
static uint32_t PI_CALL TestAddRef(void* self_ptr)
{
    return pi_refcounted_add_ref(self_ptr);
}
static uint32_t PI_CALL TestRelease(void* self_ptr)
{
 ...
 * -------------------------------------------------------------------------- */
static char const* g_argv0       = NULL;
/* ctest 传进来的真实插件名（argv[1]）：加载的成功/失败两条路径都要用它 */
static char const* g_plugin_path = NULL;
...
static int volatile g_load_error_go = 0;
...
static void LoadErrorRaceThread(void* user_data)
{
    ...
    while (!g_load_error_go)
    {
        PiPluginTestThreadYield(); /* 一起出发 */
    }
    ...
    for (round = 0; round < PI_PLUGIN_LOAD_ERROR_RACE_ROUNDS; ++round)
    {
        ...
        if (pi_plugin_module_load(path) != NULL)
        {
            ++w->bad_load;
            continue;
        }
...
static void TestHostServices(void)
{
    ...
    CHECK_EQ_INT(pi_plugin_host_services_create_default(NULL, NULL,
                                                        (PiNativeWindow)(uintptr_t)0x5678, &gui),
                 PI_OK);
    CHECK(gui != NULL);
    out = NULL;
    ...
    if (gui)
    {
        CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)gui), 0);
    }
    ...
    if (headless)
    {
        CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)headless), 0);
    }
}
...
static PiResult UnitCountingHook(void* ctx, PiGuid const* iid, void** out)
{
    (void)ctx;
    (void)iid;
    ++g_unit_hook_calls;
...
static PiResult UnitFailingHook(void* ctx, PiGuid const* iid, void** out)
{
    (void)ctx;
    (void)iid;
    if (out)
    {
        *out = (void*)&g_unit_service;
    }
    return PI_E_OUTOFMEMORY;
}

/* 谎报成功却不给指针：按"不是我的"处理，不能让插件拿到一个 NULL 的"成功"。 */
static PiResult UnitSilentHook(void* ctx, PiGuid const* iid, void** out)
{
    (void)ctx;
    (void)iid;
    (void)out;
...
static PiGuid const UNIT_UNKNOWN_IID = PI_GUID(0xDEADBEEF, 0x1234, 0x5678,
...
static void TestHostServicesCreateEx(void)
{
    ...
    if (host)
    {
        CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)host), 0);
    }
    host = NULL;
    ...
    /* --- 3) 框架自己的 IID 由框架先答掉，不转给钩子 --- */
    {
        ...
        if (counted)
        {
            CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)counted), 0);
        }
    }
    ...
    /* --- 4) 钩子的失败语义：错误码原样上抛，*out 一定回到 NULL --- */
    {
        ...
        if (failing)
        {
            CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)failing), 0);
        }
    }
    {
        IPiPluginHostServices* silent = NULL;
        CHECK_EQ_INT(pi_plugin_host_services_create_ex(NULL, NULL, PI_INVALID_WINDOW,
                                                       &UnitSilentHook, NULL, &silent),
                     PI_OK);
        ...
        if (silent)
        {
            CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)silent), 0);
        }
    }
    ...
    /* --- 5) 回归：extra_qi == NULL 与 create_default 完全一致 --- */
    {
        ...
        if (plain_ex)
        {
            CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)plain_ex), 0);
        }
        if (plain_default)
        {
            CHECK_EQ_INT(pi_iunknown_release((IPiUnknown*)plain_default), 0);
        }
    }
 ...
 * -------------------------------------------------------------------------- */
static void TestFactoryNegative(char const* plugin_path)
{
    ...
    {
        static PiGuid const UNKNOWN_CLASS_GUID =
        ...
        if (!module)
        {
            printf("  load error: %s\n", pi_plugin_module_get_load_error());
            return;
        }
