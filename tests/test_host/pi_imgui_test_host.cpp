...
static char const* g_pluginOverride     = NULL;
...
static char const* g_screenshotPath  = NULL;
...
static void           SelfTestParsePlugins(char const* list);
static DWORD WINAPI   DetachWatchdog(LPVOID param);
static bool           CaptureWindowBmp(HWND hwnd, char const* path);
static bool           CaptureHwndClientBmp(HWND hwnd, char const* path);
static IPiPluginView* CurrentView(void);

/* Set the UI status line and mirror it to the debug log. */
static void SetStatus(char const* fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
...
static void Dx11LogProc(void* user_data, char const* message)
{
    (void)user_data;
    LogStatus("%s", message);
}
...
static bool RenderFrame(bool resizeFrame, bool present, unsigned overrideW, unsigned overrideH)
{
    ...
    /* -------------------- ImGui UI -------------------- */
    {
        ...
        {
            PiPluginDescriptor const* desc = pi_plugin_host_session_get_descriptor(g_session, g_slot);
            if (desc)
            {
                    ...
                    for (uint32_t i = 0; i < desc->capability_count; ++i)
                    {
                        ...
                        char const*               kind =
                            (cap->flags & PI_PLUGIN_CAP_PROVIDES) ? "provides" :
                                          (cap->flags & PI_PLUGIN_CAP_REQUIRED) ? "requires" :
                                                                                  "optional";
    ...
    float const clear_color[4] = {0.15f, 0.15f, 0.15f, 1.0f};
...
{
    RECT startRect;
    if (!GetWindowRect(hWnd, &startRect))
    {
        return;
    }
    ...
    int const minClientW = 520, minClientH = 360;
    g_resizeFrames = 0;
    ...
    for (;;)
    {
        ...
        if (r.right - r.left < minClientW + g_frameDW)
        {
            r.left = r.right - (minClientW + g_frameDW);
        }
        if (r.bottom - r.top < minClientH + g_frameDH)
        {
            r.top = r.bottom - (minClientH + g_frameDH);
        }
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int nCmdShow)
{
    ...
    pi_plugin_host_dx11_destroy(g_dx);
    g_dx = NULL;
    ::DestroyWindow(hwnd);
    ::UnregisterClassW(wc.lpszClassName, wc.hInstance);
    ::OleUninitialize();
...
static void LoadPlugin(char const* dllPath)
{
     ...
     * copy shadowing our build output cannot hide. */
    {
        ...
        if (!hmod)
        {
            char const* base = strrchr(dllPath, '\\');
...

    {
        ...
        if (desc)
        {
            ...
            LogStatus("property com.example.kind = %s",
                      pi_plugin_descriptor_find_property(desc, "com.example.kind") ? pi_plugin_descriptor_find_property(desc, "com.example.kind") : "(absent)");
        }
        else
        {
...
static void SelfTestParsePlugins(char const* list)
{
    char const* p = (list && *list) ? list : "pi_plugin_test_plugin_qt.dll";
...
static std::string ResolvePluginPath(char const* p)
{
...
static void SelfTestStep()
{
    ...
    {
        char const* dll = g_selfTestPlugins[g_selfTestPluginIndex];
