 ...
 *
 * The framework hands a plugin an IPiPluginHostServices object. The framework's own
 * IIDs (IPiPluginHostServices / IPiPluginHostUI) are only two of the things a host may
 ...
 * install it through pi_plugin_host_services_create_ex()'s extra-QI hook, and the
 * test plugins QueryInterface the host object for PI_PLUGIN_TEST_IID_HOST_SERVICE.
 ...
 * PI_PLUGIN_CAP_OPTIONAL rather than REQUIRED.
 *
 ...
 */
#ifndef PI_PLUGIN_TEST_HOST_SERVICE_H
#define PI_PLUGIN_TEST_HOST_SERVICE_H
...
#define PI_PLUGIN_TEST_MSG_HOST_SERVICE ((uint32_t)0x8002u)

typedef struct IPiPluginTestHostServiceVtbl {
    IPiUnknownVtbl base;
...
#endif /* PI_PLUGIN_TEST_HOST_SERVICE_H */
