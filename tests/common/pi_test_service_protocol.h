 ...
 * `pi_plugin_service_start()` expects. Both sides include this header so the strings
 ...
 */
#ifndef PI_PLUGIN_TEST_SERVICE_PROTOCOL_H
#define PI_PLUGIN_TEST_SERVICE_PROTOCOL_H

#include "piplugin/pi_plugin.h"

/* poll() 报数：wparam = 第几次 tick。宿主据此断言"poll 真的在干活"。 */
#define PI_PLUGIN_TEST_MSG_SERVICE_TICK ((uint32_t)0x8003u)

/* stop() 报数：wparam = 第几次 stop 调用。宿主据此断言 stop 的幂等性，
 * 以及卸载序列确实又调了一次 stop。 */
#define PI_PLUGIN_TEST_MSG_SERVICE_STOP ((uint32_t)0x8004u)

/* start() 的选项键：`slot` 必填（缺了就按文档返回 PI_E_MISSINGCAPABILITY），
 * `interval` 可选（每 N 次 poll 报一次 tick，缺省 1）。 */
#define PI_PLUGIN_TEST_SERVICE_OPTION_SLOT     "slot"
#define PI_PLUGIN_TEST_SERVICE_OPTION_INTERVAL "interval"

#endif /* PI_PLUGIN_TEST_SERVICE_PROTOCOL_H */
