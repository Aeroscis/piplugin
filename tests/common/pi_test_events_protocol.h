 ...
 */
#ifndef PI_TEST_EVENTS_PROTOCOL_H
#define PI_TEST_EVENTS_PROTOCOL_H
...
#define PI_TEST_EVENTS_TOPIC_READY         "com.example.plugin.ready"
/* host -> plugin: "welcome", delivered by ADDRESS (the plugin's sink) */
#define PI_TEST_EVENTS_TOPIC_WELCOME       "com.example.host.welcome"
/* plugin -> host: the reply it publishes from inside its sink */
#define PI_TEST_EVENTS_TOPIC_ACK           "com.example.plugin.ack"
/* host -> everyone: "broadcast", delivered by TOPIC (subscriptions) */
#define PI_TEST_EVENTS_TOPIC_BROADCAST     "com.example.host.broadcast"
/* plugin -> host: what the plugin saw on that broadcast (echoes the count) */
#define PI_TEST_EVENTS_TOPIC_SAW_BROADCAST "com.example.plugin.saw_broadcast"
/* host -> nobody: used to check that unsubscribe really stops delivery */
#define PI_TEST_EVENTS_TOPIC_NOBODY        "com.example.nobody.listens"

/* Payload keys */
#define PI_TEST_EVENTS_KEY_PLUGIN   "plugin"    /* ready:   which plugin it is    */
#define PI_TEST_EVENTS_KEY_GREETING "greeting"  /* welcome: what the host says    */
#define PI_TEST_EVENTS_KEY_ACKED    "acked"     /* ack:     always "1"            */
#define PI_TEST_EVENTS_KEY_COUNT    "count"     /* saw_broadcast: how many seen   */
#define PI_TEST_EVENTS_KEY_N        "n"         /* broadcast: which broadcast it is */
...
static inline const char* pi_test_event_payload(const PiEvent* event, const char* key)
{
    uint32_t i;
...
#endif /* PI_TEST_EVENTS_PROTOCOL_H */
