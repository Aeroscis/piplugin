...
static char const*            g_paths[PI_PLUGIN_MULTI_PLUGIN_COUNT];
                            ...
                            uintptr_t wparam, intptr_t lparam)
{
    (void)user_data;
    (void)lparam;
    if (msg == PI_PLUGIN_QT_HEARTBEAT_A)
    {
        ++g_heartbeats[0];
    }
    else if (msg == PI_PLUGIN_QT_HEARTBEAT_B)
    {
        ++g_heartbeats[1];
    }
    ...
    if (g_probe_mode)
    {
        int const on_main = ((unsigned long)GetCurrentThreadId() == g_main_thread_id);
...
static void LayoutContainers(void)
{
    RECT rc;
    int  w, h;
    if (!g_window || !GetClientRect(g_window, &rc))
    {
        return;
    }
...
static int CreateHostShell(int container_count, int ui_container_index)
{
    ...
    if (PI_FAILED(pi_plugin_host_services_create_default(
            ...
            (ui_container_index >= 0) ? (PiNativeWindow)g_containers[ui_container_index] : PI_INVALID_WINDOW,
            &g_services)))
    {
...
static void CheckPluginUi(unsigned index)
{
    ...
    PiPluginDescriptor const* desc = pi_plugin_host_session_get_descriptor(g_session, g_slots[index]);
    HWND                      plugin_window;
    HWND                      parent;
    RECT                      r;
 ...
 * -------------------------------------------------------------------------- */
static int RunMultiPluginMode(char const* path_a, char const* path_b)
{
    int frame;
    ...
    if (CreateHostShell(PI_PLUGIN_MULTI_PLUGIN_COUNT, 0) != 0)
    {
        return 1;
    }
     ...
     * STATIC 时不存在这个模块（每个插件各带一份）。 */
    {
        ...
        if (!kit)
        {
            kit = ::GetModuleHandleA("piplugin_qt.dll");
        }
        LogStatus("qt adapter kit: %s", kit ? "loaded as a shared module (one copy for the process)" : "not a separate module (each plugin has its own copy)");
    }
    ...
    for (int i = 0; i < PI_PLUGIN_MULTI_PLUGIN_COUNT; ++i)
    {
        if (g_slots[i] == PI_PLUGIN_HOST_SESSION_INVALID_SLOT)
        {
            continue;
        }
        if (g_heartbeats[i] == 0)
...
static int RunPostThreadMode(const char* dll_path)
{
    ...
    if (g_report_msgs == 0)
    {
        Fail("pi_plugin_qt_view_post never ran the callback within %d ms "
             "(queued call was dropped instead of marshalled)",
             PI_PLUGIN_PROBE_TIMEOUT_MS);
        ok = 0;
    }
    else if (g_report_on_ui_thread == 0)
    {
        Fail("pi_plugin_qt_view_post ran the callback on the CALLING thread (%d report(s), "
             "none on the host GUI thread) - it did not marshal",
             g_report_msgs);
        ok = 0;
    }
...
static void ResizeContainer(HWND container, int x, int y, int w, int h)
{
    if (container)
    {
        ::SetWindowPos(container, NULL, x, y, w, h, SWP_NOZORDER);
    }
}
...
static void CheckEmbedded(char const* stage, IPiPluginView* view, HWND expected_container,
 ...
 * attach 到新容器 -> 泵几帧 */
static int SwitchContainerTo(IPiPluginView* view, HWND target, char const* stage)
{
...
static int RunContainerSwitchMode(char const* dll_path)
{
    IPiPluginView* view = NULL;
    ...
    g_slots[0] = PI_PLUGIN_HOST_SESSION_INVALID_SLOT;
    if (CreateHostShell(2, 0) != 0)
    {
        return 1; /* 两个容器；宿主服务先报告 A */
    }
    ...
    /* 2) 切到 B */
    if (SwitchContainerTo(view, container_b, "switch A->B") != 0)
    {
        ok = 0;
    }
    else
    {
        CheckEmbedded("switch A->B", view, container_b,
                      ClientWidth(container_b), ClientHeight(container_b));
    }

    /* 3) 再切回 A */
    if (SwitchContainerTo(view, container_a, "switch B->A") != 0)
    {
        ok = 0;
    }
    else
    {
        CheckEmbedded("switch B->A", view, container_a,
                      ClientWidth(container_a), ClientHeight(container_a));
    }

    /* 4) 尺寸往返：小 -> 大 -> 小，容器与 view 一起变，每一步都断言几何 */
    {
        static int const kSizes[3][2] = {
            {320, 240},
            {780, 500},
            {360, 280}
        };
        ...
        for (int i = 0; i < 3; ++i)
        {
            int const w = kSizes[i][0], h = kSizes[i][1];
...
static int RunImguiPairMode(const char* path_a, const char* path_b)
{
     ...
     * 独立的套件模块。打印出来是为了让日志能区分"没共享"和"共享了一份"。 */
    {
        ...
        LogStatus("imgui adapter kit: %s", kit ? "loaded as a separate module" : "statically linked into each plugin (one copy per module)");
    }
    ...
    for (int i = 0; i < PI_PLUGIN_MULTI_PLUGIN_COUNT; ++i)
    {
        ...
        if (g_imgui_frames[i] == 0)
        {
            Fail("plugin[%d] never reported a rendered frame - its draw callback was not "
                 "driven while the other plugin was alive",
                 i);
            ok = 0;
        }
        if (g_imgui_last_tick[i] < 2)
        {
            Fail("plugin[%d] reported only frame %llu - it rendered the first frame and "
                 "then stopped",
                 i, (unsigned long long)g_imgui_last_tick[i]);
            ok = 0;
        }
