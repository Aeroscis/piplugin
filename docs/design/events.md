...
| 方向 | 现状 | 限制 |
|---|---|---|
| 插件 → 宿主 | `pi_plugin_host_post_message(msg, wparam, lparam)` | 只有 3 个整数，没有主题、没有结构化负载、无法路由到"关心这件事的人" |
| 宿主 → 插件 | 只有 `pi_plugin_on_idle()` / `pi_plugin_on_resize()` 这类**轮询式视图回调** | 宿主想说一句话，只能等插件下一帧来问；插件没有 view（headless service 插件）时连这点都没有 |
...
- G3 能力协商：宿主/插件任一侧不实现都能优雅降级（LV2 风格：QI 探测失败即跳过），
  与 `IPiPluginHostUI` / `IPiPluginService` 完全同构；
...
- N3 不做请求/响应（RPC）语义——若将来要，属于新接口（`IPiPluginEventQuery`），不塞进本期；
- N4 不做沙箱/权限（FUT-06）；
- N5 不给 `pi_plugin_host_post_message` 加语义，也不废弃它：它继续是"最原始的拉杆"，事件是结构化通道。
...
- 比较按字节、大小写敏感（与 `pi_plugin_descriptor_find_property` 一致）。
...
```c
#define PI_PLUGIN_EVENT_NOTIFY     0x00000001u  /* 通知：发生了什么（默认） */
#define PI_PLUGIN_EVENT_REQUEST    0x00000002u  /* 请求：希望对方做点什么（仍不承诺回应） */
...
| 选项 | 优点 | 缺点 |
|---|---|---|
| **A. 宿主 marshal 到自己的主线程**（建议，roadmap 建议） | 插件侧 sink 不需要任何锁；与 GUI/句柄线程模型一致（Win32 父子窗口同线程的教训）；与 `pi_plugin_host_post_message` 的"宿主决定 marshal"完全对称 | 宿主必须实现队列；投递延迟到下一次泵 |
...
1. 宿主保证 `pi_plugin_event_deliver()` **在宿主自己的主/UI 线程**上被调用（host kit 的
   `pi_plugin_host_session_drive_idle()` 是天然泵点，但泵的时机仍归宿主）；
2. 因此插件 sink 里可以直接操作 UI/句柄，**不需要锁**；
3. 宿主可以丢弃、合并、批量投递（尽力而为），但**不得**在插件 `pi_plugin_terminate()` 之后再投递。

### 3.5 订阅模型

- **插件 → 宿主发布**：`pi_plugin_host_events_publish()`，由宿主路由；
- **宿主/插件 → 插件投递**：宿主在泵点调用该插件 sink 的 `pi_plugin_event_deliver()`；
- **谁是订阅者**：三个可能 —— 宿主自己的代码、其它插件、以及"框架"（日志/调试）。
  为了不把路由策略写死，`IPiPluginHostEvents` 只提供 **publish/subscribe/unsubscribe** 三件事，
  由宿主决定路由策略（广播 / 按 topic 定向 / 只喂自己）。

**可选糖**：提供一个很小的 `PiPluginEventRouter`（静态库 `piplugin_events`，非必须链接），
...
- **有界队列**：队列满时宿主**丢弃最旧**或**丢弃最新**由宿主决定（日志里必须体现），
  框架不定义；插件不能依赖"一定能收到"；
- **重入**：在 `pi_plugin_event_deliver()` 里再 `publish()` 是**允许**的（入队，不在回调里递归投递），
  但要写进契约（否则插件作者会以为可以同步拿到结果）；
- **生命周期**：`PiPluginEvent` 与其 payload 只在调用期间有效（借用）。
 ...
 *   PI_PLUGIN_IID_EVENT_SINK   data1 = 0x00000030   （插件可选实现）
 *   PI_PLUGIN_IID_HOST_EVENTS  data1 = 0x00000031   （宿主可选提供）
 */
...
/* ---- 插件侧：宿主投递事件给它 ---- */
typedef struct IPiPluginEventSinkVtbl {
    IPiUnknownVtbl base;
...
/* + inline 帮助 pi_plugin_event_deliver()，NULL 安全（无 sink -> PI_E_NOINTERFACE） */
...
可选糖（`host_kits/events/`，静态库 `piplugin_events`，宿主可不用它）：`PiPluginEventRouter`
实现上面这套接口 —— 订阅表 + 有界队列（默认 256，满则丢新并计数）+ `pi_plugin_event_router_pump()`
+ `pi_plugin_event_router_stats()`，并直接提供 `pi_plugin_event_router_extra_qi()` 作为
`pi_plugin_host_services_create_ex()` 的钩子（于是插件 QI 宿主对象就能拿到它，见 §8.2）。

宿主 kit L0 侧只加三件事（机制，不含策略）：`pi_plugin_host_session_has_event_sink()`、
`pi_plugin_host_session_deliver_event()`（投给某槽位 —— 投什么、投给谁是宿主的策略）、
`pi_plugin_host_session_get_host_events()`（借用），外加卸载序列里的"退订 + 释放 sink"两步。

既有机制如何"自动"覆盖需求（无需新 API）：

- **能力声明**：插件 `PROVIDES PI_PLUGIN_IID_EVENT_SINK`、`OPTIONAL PI_PLUGIN_IID_HOST_EVENTS`；
  宿主用既有的 `pi_plugin_host_session_require(&PI_PLUGIN_IID_EVENT_SINK)` 就能实现
  "我生态内所有插件必须能被通知"；
- **优雅降级**：宿主 QI 插件要 sink，失败就跳过（不报错）；
  插件 QI 宿主 `PI_PLUGIN_IID_HOST_EVENTS` 失败就用 `post_message`/轮询兜底；
...
宿主                                  插件
────                                  ────
QI(plugin, PI_PLUGIN_IID_EVENT_SINK) ───────► 提供 sink（或返回 NOINTERFACE）
  ◄─────────────────────────────────── PI_OK / 无
pi_plugin_event_deliver({type=REQUEST,
                  topic="com.example.refresh"})   ──►  插件在 sink 里处理
                                                        插件 publish({"com.example.progress",
                                                                      {"percent","42"}})
  ◄── pi_plugin_host_events_publish(...)  宿主路由：自己处理 / 转投给别的插件 sink
```

测试计划（实现阶段）：

1. `tests/test_plugin_events/`：插件 `PROVIDES PI_PLUGIN_IID_EVENT_SINK`（记录收到的事件并回发），
   并在 `initialize` 后 QI 宿主 `IPiPluginHostEvents` 发布一条事件；
2. headless 测试宿主实现 `IPiPluginHostEvents`（用一个最小路由 + 队列），
...
- **不改任何现有 vtbl**（G4）：`IPiPluginEventSink` / `IPiPluginHostEvents` 是两个全新接口；
- **数据面**：`PiPluginEvent` 是新类型，不动 `PiPluginDescriptor`；
...
| # | 决策点 | 结论 | 落地位置 |
|---|---|---|---|
| D1 | 本期是否两个方向都做 | **两个方向都做**：插件 sink（寻址投递）+ 宿主事件（发布/订阅） | `IPiPluginEventSink`、`IPiPluginHostEvents` |
| D2 | topic 标识 | **UTF-8 字符串**；`pi.` 框架保留、app 用自有前缀；字节比较、大小写敏感、无通配语法 | `pi_plugin_events.h` 顶部契约；`PI_PLUGIN_EVENT_TOPIC_MAX`=128 |
