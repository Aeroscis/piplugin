...
1. 不创建顶层窗口、不决定布局、不选择父窗口 —— 容器是**宿主**创建并交给 `pi_plugin_attach(parent)` 的；
...
| 槽位 | 必须做到 | 常见错误 |
|---|---|---|
| `pi_query_interface` | 认领 `PI_IID_UNKNOWN` 与 `PI_PLUGIN_IID_PLUGIN_VIEW`，**返回时已 AddRef**；失败时 `*out = NULL` | 返回未 AddRef 的指针（宿主一定会 release） |
| `pi_add_ref` / `pi_release` | 用 `PiRefCountedBase` 实现；归零时同步销毁（见 §6） | 在别的线程里异步析构 |
| `pi_plugin_attach(parent)` | **第一次**才创建资源；成功返回 `PI_OK`；已 attach 再调返回失败（不要静默重复创建） | 在 create 时就建窗口（此时还没有容器） |
| `pi_plugin_detach()` | **幂等**；返回时资源全部销毁；之后**可以再次 attach** | 只隐藏窗口、不销毁；或 detach 后留下悬垂 HWND |
| `pi_plugin_get_native_window()` | 返回**借用**句柄；未 attach 时返回 `PI_INVALID_WINDOW` | 在这里创建窗口 |
| `pi_plugin_on_resize(w, h)` | 让插件内容跟随宿主给的尺寸；未 attach 时是安全的空操作 | 自己决定尺寸、忽略宿主给的 w/h |
| `pi_plugin_on_idle()` | 宿主每帧（或每个循环）调一次；把"一帧"跑完；未 attach 时安全 | 在里面阻塞、或自己起定时器决定节奏 |
| `pi_plugin_get_preferred_size()` | 有真意见就给建议值；**没有就返回 `PI_E_NOTIMPL`** | 硬编码一个数字并返回 `PI_OK`（见 freeze review F7） |
| `pi_plugin_set_visible(v)` | attach 之后可切换可见性；未 attach 时安全 | 把可见性与"是否创建"混为一谈 |

**宿主侧的两条义务**（写在这里，因为套件可以依赖它们）：

1. 每帧调用一次 `pi_plugin_on_idle()`（这是插件 UI 的心跳）；
2. 卸载插件前先 `pi_plugin_detach()` + `pi_release()`；套件的 `pi_plugin_terminate` 兜底钩子（§7）在。
...
宿主编译期            插件模块
────────────────────────────────────────────────────────────
pi_plugin_module_load        ← 只有代码被映射，什么都还没创建
pi_plugin_factory_create_instance
pi_plugin_initialize
pi_plugin_get_view    → 套件 create：**只造对象，不造资源**
pi_plugin_view_attach(容器)  → 套件在这里创建第一个窗口/设备/控件
   … 每帧 pi_plugin_on_idle / 尺寸变化 pi_plugin_on_resize …
pi_plugin_view_detach        → 套件销毁资源（同步！）
pi_iunknown_release   → 引用归零：若仍 attach，先内部 detach，再释放对象
pi_plugin_terminate   → 插件调套件的 shutdown 兜底（幂等）
工厂/实例 release
pi_plugin_module_unload      ← 到这里，套件在模块里的一切必须已经不存在
```
...
3. **detach → attach 必须可重复**：宿主可以反复挂载/卸载同一实例（多插件宿主、
   布局切换都会这么做）。
4. **对象与资源分开**：view 对象在 `pi_plugin_get_view()` 之后就可能被宿主持有；
   资源只在 attach 期间存在。
...
| 模型 | 含义 | 套件必须满足 | 插件侧的代价 |
|---|---|---|---|
| **A. 全宿主线程**（推荐） | 套件一切调用都在宿主调 `pi_plugin_on_idle()` 的那条线程上 | 没有私有线程；回调直接同步执行 | 插件无需加锁，可直接碰 UI/句柄 |
| **B. 私有线程 + marshal** | 套件自建线程跑工具包的消息循环，宿主调用 marshal 过去 | 必须保证：`pi_plugin_detach()` 返回时控件已销毁；`pi_release()` 返回时线程已 join | 插件回调在私有线程上，插件必须自己加锁 |
...
- `piplugin_qt`：**曾经**是模型 B，并且**因此付过代价**——后台线程跑
  ...
  detach 死锁。现在也是模型 A（`QApplication` 建在宿主 GUI 线程上，`pi_plugin_on_idle()` 里
...
**容器**：`pi_plugin_attach(parent)` 给的 `PiNativeWindow` 是宿主创建的容器。套件要做的只有
"让我的控件成为它的子窗口/嵌入其中"：
...
**尺寸**：只信 `pi_plugin_on_resize(w, h)` 给的数字，别去读容器的实际大小当唯一真相
（宿主可能在布局发生前就通知你）。

## 6. 每一帧做什么（`pi_plugin_on_idle`）
...
| 形态 | 适用 | 要求 |
|---|---|---|
| **STATIC** | 套件没有进程级状态（或状态天然只属于一个模块），如 `piplugin_imgui` | 源码随插件编译；注意**每个插件 DLL 一份**静态数据——所以"两个插件会不会互相踩"这类进程级问题，只有**两个不同的插件模块**同进程才暴露得出来（历史上 imgui 套件正是这条路崩过：进程级窗口类名只有一份）。回归必须用两个模块，见 `conformance.md` |
| **SHARED** | 套件持有进程级状态（唯一 `QApplication`、全局登记表），如 `piplugin_qt`（APP-08） | 显式导出（`PI_PLUGIN_QT_API` 这类宏）、部署时把套件 DLL 放在插件旁边、**不能靠"每模块一份"来隔离状态** |

判定口诀：**"两个插件同时加载"时会不会互相踩？** 会 → SHARED，并且套件要提供
按 owner 作用域的销毁入口（`pi_plugin_qt_view_shutdown_owner(owner)`），
...
- 环境变量：`PI_<KIT>_TRACE=1`（例如 `PI_PLUGIN_QT_VIEW_TRACE`、`PI_PLUGIN_WIN32_VIEW_TRACE`）；
...
- [ ] `pi_plugin_attach` 之前没有创建任何原生资源（`pi_plugin_get_view()` 只造对象）；
- [ ] `pi_plugin_detach()` 幂等、同步、之后可再次 attach；
- [ ] `pi_release()` 归零时把所有资源同步销毁；
- [ ] 提供了幂等的 `pi_<kit>_shutdown()`，并在 §7 的意义下注销了窗口类；
- [ ] 头文件里写明了线程模型（A 或 B）与 user_data 的 retain/release 语义；
- [ ] `pi_plugin_get_preferred_size()` 没有硬编码谎言（要么真意见，要么 `PI_E_NOTIMPL`）；
- [ ] 每帧工作有上限、不阻塞宿主；
...
| shutdown 兜底 | 无（无跨模块状态） | `pi_plugin_qt_view_shutdown_owner()` | `pi_plugin_win32_view_shutdown()` |
| `get_preferred_size` | 硬编码 400×300（F7，待改） | 硬编码 400×300（F7，待改） | **`PI_E_NOTIMPL`（推荐做法）** |
| trace | 无 | `PI_PLUGIN_QT_VIEW_TRACE=1` | `PI_PLUGIN_WIN32_VIEW_TRACE=1` |
