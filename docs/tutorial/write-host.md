...
pi_plugin_host_services_create_default(&MessageProc, NULL,
                                /* GUI 宿主传嵌入窗口句柄；无头传 PI_INVALID_WINDOW */,
                                &host);

/* 2) 加载插件模块 */
PiPluginModule* module = pi_plugin_module_load("my_plugin.dll");
if (!module) { /* 查看 pi_plugin_module_get_load_error()（线程局部：本线程那次的结果） */ }

/* 3) 取工厂 */
IPiPluginFactory* factory = NULL;
pi_plugin_module_get_factory(module, &factory);

/* 4) 能力门检查（可选但推荐，实例化前） */
const PiPluginDescriptor* desc = NULL;
pi_plugin_factory_get_descriptor(factory, &desc);
if (pi_plugin_descriptor_requires(desc, &PI_PLUGIN_IID_HOST_UI)) {
...
/* 5) 创建并初始化插件实例 */
PiGuid classGuid;
pi_plugin_factory_get_class_guid(factory, 0, &classGuid);
IPiPluginBase* plugin = NULL;
pi_plugin_factory_create_instance(factory, &classGuid, host, &plugin);
...
if (PI_SUCCEEDED(pi_plugin_get_view(plugin, &view)) && view) {
    pi_plugin_view_attach(view, (PiNativeWindow)embedContainerHandle);
...
while (running) {
    …处理宿主自己的事件…
    if (view) pi_plugin_view_on_idle(view);            /* 每帧 pump 插件 */
    if (resized) pi_plugin_view_on_resize(view, w, h);
}

/* 8) 卸载：先释放插件，再卸载模块 */
if (view)  { pi_plugin_view_detach(view); pi_iunknown_release((IPiUnknown*)view); }
...
插件可随时通过 `pi_plugin_host_post_message(host, msg, wparam, lparam)` 向宿主发消息
（任何线程）。宿主在创建服务对象时注册回调：
...
pi_plugin_host_services_create_default(&MessageProc, /*user_data=*/NULL, window, &host);
```
...
- imgui 宿主：主循环里 `PeekMessage` 循环之后调用 `pi_plugin_view_on_idle(view)`。
- Qt 宿主：`QTimer`（interval 0）在每次事件循环迭代触发 `pi_plugin_view_on_idle(view)`。

### 3.3 转发尺寸变化

- Win32：`WM_SIZE` 里调整容器并调用 `pi_plugin_view_on_resize(view, w, h)`。
- Qt：给容器安装 `eventFilter`，`QEvent::Resize` 时转发。

## 4. Headless 宿主要点

- `ui_parent_window` 传 `PI_INVALID_WINDOW` → 宿主**不暴露** `IPiPluginHostUI`，
  插件的 `QueryInterface(PI_PLUGIN_IID_HOST_UI)` 返回 `PI_E_NOINTERFACE` → 插件不建 UI。
- 实例化前做能力门：`pi_plugin_descriptor_requires(desc, &PI_PLUGIN_IID_HOST_UI)` 为真 → 拒绝加载。
- 可以探测 `PI_PLUGIN_IID_SERVICE`：headless 服务器宿主加载提供服务的插件并用
  `pi_plugin_service_start` / `pi_plugin_service_poll` / `pi_plugin_service_stop` 驱动。参照实现是
  ...
  - `start()` 的必填选项缺失时返回 `PI_E_MISSINGCAPABILITY`（这是接口文档的承诺，
    不是 `PI_FAIL`）；
  - `poll()` 由**宿主的主循环**驱动（对应 GUI 插件的 `pi_plugin_on_idle`）；服务未运行时
    应当明确失败，而不是假装在工作；
  - `stop()` 幂等：卸载序列会再调一次，插件必须能承受；
  - 纯服务插件不必实现 `IPiPluginView`：`pi_plugin_get_view` 返回 `PI_E_NOINTERFACE` 即可。

## 5. 便捷 API：pi_plugin_host_create_plugin
...
IPiPluginBase* plugin = NULL;
pi_plugin_host_create_plugin("my_plugin.dll", &classGuid, host, &plugin, &module);
/* 用完：
...
pi_plugin_host_session_set_logger(session, OnSessionLog, NULL);  /* 步骤日志去哪由宿主决定 */
...
if (PI_FAILED(pi_plugin_host_session_load(session, "my_plugin.dll", &slot))) {
    printf("%s\n", pi_plugin_host_session_last_error(session));  /* 含拒绝理由 */
    return;
}

/* 容器由宿主自己创建并摆位；kit 只接收它 */
pi_plugin_host_session_attach_view(session, slot, (PiNativeWindow)myContainer, /*set_visible=*/1);

while (running) {
    …处理宿主自己的事件…
    pi_plugin_host_session_drive_idle(session);                  /* 每帧 pump；时机归宿主 */
    pi_plugin_view_on_resize(pi_plugin_host_session_get_view(session, slot), w, h);
}

pi_plugin_host_session_unload(session, slot);   /* 七步序列，顺序由 kit 保证 */
...
- 需要"实例化前按 descriptor 过滤"时用 `pi_plugin_host_session_inspect()` +
  `pi_plugin_host_session_instantiate()`（`load()` 就是这两步合起来）；
- 门禁是**双向**的：插件 `PI_PLUGIN_CAP_REQUIRED` 的能力宿主给不出就拒绝；宿主
  `pi_plugin_host_session_require()` 声明的能力插件没 `PROVIDES` 也拒绝；
...
HWND hwnd = CreateWindowW(cls, title, WS_OVERLAPPEDWINDOW | pi_plugin_host_dx11_top_level_style(), ...);

PiPluginHostDx11Desc desc = {};
...
pi_plugin_host_dx11_create((PiNativeWindow)hwnd, &desc, &g_dx);

/* 容器：矩形你说了算 */
g_embedContainer = (HWND)pi_plugin_host_dx11_create_embed_container((PiNativeWindow)hwnd, 410, 0, w, h);
...
...
pi_plugin_host_dx11_prepare_size(g_dx, clientW, clientH);   /* WM_SIZE 里转发；策略在 kit 内 */
pi_plugin_host_dx11_present(g_dx, 1);
```
...
框架只保证两个宿主 IID：`IPiPluginHostServices` 与（GUI 宿主才有的）`IPiPluginHostUI`。
app 要给插件自己的服务（配置中心、日志、任务队列、许可证……）时，用
`pi_plugin_host_services_create_ex()` 装一个 **extra-QI 钩子**：框架 IID 之外的
...
IPiPluginHostServices* host = NULL;
pi_plugin_host_services_create_ex(&MessageProc, /*user_data=*/NULL,
...
if (PI_SUCCEEDED(pi_iunknown_query_interface((IPiUnknown*)host, &MY_SERVICE_IID, (void**)&svc))) {
    ...
    /* 宿主没有这个服务：照常运行。声明为 PI_PLUGIN_CAP_OPTIONAL 就是这个意思。 */
}
```

要点：

- **插件侧零新增 API**：这正是它相对 §6 的 `pi_plugin_host_session_require()`（通道 A，
  宿主消费插件定义的接口）的镜像；
- 钩子可能被插件的任意线程调用（插件可以从自己的线程 QI），要线程安全；
- 有窗口时的 `IPiPluginHostUI`、以及 `IPiPluginHostServices` 本身由框架先答掉，不会转到钩子；
  **headless 的 `IPiPluginHostUI` 算未命中**，所以 app 也可以借此提供自己的 UI 服务；
...
static PiPluginPtr<IPiPluginHostServices> g_hostServices;   /* 析构即 release */

/* 创建：put() 给出参地址，句柄接管 create_default 返回的那一份引用 */
pi_plugin_host_services_create_default(&MessageProc, nullptr, window, g_hostServices.put());
pi_plugin_host_session_create(g_hostServices.get(), &session);
...
- `PiPluginUniqueModule`：RAII `pi_plugin_module_load` / `pi_plugin_module_unload`（`PiPluginUniqueModule::load(path, m)`
  + `module.factory()`），声明顺序即加载顺序时，C++ 的逆序析构正好等于七步卸载序列；
- `PiPluginPtr<T>::qi_to<U>()`：按 `PiPluginIidOf<U>` 的框架 IID 做一次 QI，失败返回空句柄；
- `pi_plugin_cpp_destroy<T>`：`PiRefCountedBase` 需要的 C++ 析构 thunk。

`PiPluginPtr<T>` **接管**已有引用（框架返回的接口指针一律已 AddRef），借用指针
（descriptor / native window）禁止包进去；拷贝被删除，要第二份持有就写
`PiPluginPtr<T>::add_ref(p)`。参照实现：`tests/test_host_qt`（宿主服务对象）、
...
| 想要 | 用什么 | 谁决定收件人 |
|---|---|---|
| 宿主通知**某个**插件 | 插件实现 `IPiPluginEventSink`，宿主 `pi_plugin_host_session_deliver_event()` | 宿主（按地址） |
| 按主题一对多 / 插件之间间接协作 | 宿主提供 `IPiPluginHostEvents`，各方 `subscribe` | 订阅关系（按 topic） |
...
static PiPluginEventRouter* g_router;

/* 1) 先建路由器，再用它当宿主对象的 extra_qi 钩子（通道 B）：插件 QI 就能拿到 */
pi_plugin_event_router_create(&g_router);
pi_plugin_host_services_create_ex(&MessageProc, NULL, window,
                           &pi_plugin_event_router_extra_qi, g_router, &services);
pi_plugin_host_session_create(services, &session);          /* session 也会 QI 到它 */

/* 2) 宿主自己订阅（owner = NULL 表示"宿主自己"） */
IPiPluginHostEvents* events = pi_plugin_event_router_host_events(g_router);
uint32_t h = 0;
pi_plugin_host_events_subscribe(events, "com.example.plugin.ready", NULL, OnReady, ctx, &h);

/* 3) 加载 + 投递（按地址；插件没有 sink 时返回 PI_E_NOINTERFACE，跳过即可） */
pi_plugin_host_session_load(session, "my_plugin.dll", &slot);
if (pi_plugin_host_session_has_event_sink(session, slot)) {
    PiPluginProperty p = { "greeting", "hello" };
    PiPluginEvent ev = { 0 };
    ev.type = PI_PLUGIN_EVENT_REQUEST; ev.topic = "com.example.host.welcome";
    ev.payload = &p; ev.payload_count = 1;
    pi_plugin_host_session_deliver_event(session, slot, &ev);   /* 宿主主线程 */
}

/* 4) 主循环里泵一次：把插件发布的事件分发给订阅者（时机归宿主） */
pi_plugin_event_router_pump(g_router);
```

规则（细则见 `docs/design/events.md` 与 `interfaces.md` 2.7/2.8）：

- `publish` 任意线程可调；**投递与订阅回调都在宿主主线程**，所以插件 sink 不需要锁；
- 事件是**尽力而为**的：可丢、可合并；`pi_plugin_event_router_stats()` 里的 `dropped_full`
  就是"丢了几个"的可观测证据；
...
- [ ] 创建 `IPiPluginHostServices`（GUI 传容器窗口 / headless 传 `PI_INVALID_WINDOW`）
- [ ] 要给插件自己的服务时用 `pi_plugin_host_services_create_ex()` 装 extra-QI 钩子（§8）
- [ ] `pi_plugin_module_load` 失败时检查 `pi_plugin_module_get_load_error()`（线程局部；要留存原因用 `pi_plugin_module_get_load_error_r(buf, size)`）
- [ ] 实例化前做能力门检查（`pi_plugin_descriptor_requires`）
- [ ] `pi_plugin_get_view` 成功后 attach + set_visible
- [ ] 主循环每帧 `pi_plugin_on_idle`
- [ ] 尺寸变化转发 `pi_plugin_on_resize`
- [ ] 卸载顺序：detach view → release view → terminate/release plugin → release factory → unload module → release host
- [ ] 需要事件时：建路由器并当 extra_qi 钩子挂上，主循环里 `pi_plugin_event_router_pump()`（§10）
- [ ] C++ 宿主：宿主服务对象与模块用 `PiPluginPtr` / `PiPluginUniqueModule` 持有，省掉手写 release（§9）