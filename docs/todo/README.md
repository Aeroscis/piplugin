...
| 构建打包 | 仓库内置 CMakePresets（W-09）、cpack 产出（W-08）、~~Linux CI job~~（**已完成 W-10**：ubuntu + gcc/clang（**不用 conan**——Qt/imgui 关掉后没有第三方依赖），job 内逐条写明不覆盖什么；见 `build.md` #2）、~~`CMAKE_INSTALL_PREFIX` 未设置导致 INSTALL 失败~~（**已修复**：`bin/<Config>` 改由 POST_BUILD 维护，前缀固定到 `build/install`，见 `install-design-review-prompt.md`） |
